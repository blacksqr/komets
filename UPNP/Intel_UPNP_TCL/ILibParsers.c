...
void ILibPushStack(void **TheStack, void *data)
{
	...
	RetVal->Data = data;
	RetVal->Next = (struct ILibStackNode *)*TheStack;
	*TheStack = RetVal;
}
...
struct HashNode* ILibFindEntry(void *hashtree, void *key, int keylength, int create)
{
	...
	while(done==0)
	{
		...
		else if(create!=0)
		{
			...
			current->Next->KeyValue = (char*)MALLOC(keylength);
...
void ILibLifeTime_Add(void *LifetimeMonitorObject,void *data, int seconds, void* Callback, void* Destroy)
{
	...
	ltms->CallbackPtr = (void (*)(void*))Callback;
	ltms->DestroyPtr  = (void (*)(void*))Destroy;
	ltms->Next = NULL;
