...
#_________________________________________________________________________________________________________
method Comet_element Update_placement {} {}

#_________________________________________________________________________________________________________
#_________________________________________________________________________________________________________
#_________________________________________________________________________________________________________
method Comet_element get_L_ancestors {L_name} {
 upvar $L_name L
 
 foreach m [this get_mothers] {$m get_L_ancestors L}
 lappend L $objName
}

#_________________________________________________________________________________________________________
method Comet_element get_L_out_descendants {L_name} {
 upvar $L_name L
 
 lappend L $objName
 foreach m [this get_out_daughters] {$m get_L_out_descendants L}
}
...
proc Update_style {dsl_q dsl_css L_fct CSS current {L_mapping ""} {L_rep ""}} {
 ...
 foreach r $CSS {
   ...
   #puts " sel : $sel"
   lappend parsed_CSS [list [Style_CSSpp Parse $sel] $att]
  }
 
 return [Update_style_parsed_CSS $dsl_q $dsl_css $L_fct $parsed_CSS $current $L_mapping $L_rep]
}
...
#puts "$objName Physical_model::Substitute_by_PM_factory {$L_PM_f}"
 set LM [this get_LM]
# Chercher d'abord dans les PM non actifs !!! et rendre inactif une fois substitué !!!!
...
#_________________________________________________________________________________________________________
# XXX TODO
# Trouver un bon système pour ne pas ré appliquer des transfo indéfiniment
# Trouver un système pour faire de l'encapsulation vraiment automatique, en générant les méthodes de l'API sémantique
